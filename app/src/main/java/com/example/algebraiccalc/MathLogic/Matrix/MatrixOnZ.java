...
public class MatrixOnZ extends Matrix{
    public MatrixOnZ(){ }
    public MatrixOnZ(ArrayList<ArrayList<Fraction>> arrayList){
    ...
    public Matrix add(Matrix matrix) {
        ...
        for(int i=0;i<getCountOfLine();i++){
            ...
            for(int j=0;j<getCountOfColumn();j++){
                part_new.add(this.get(i,j).get(0).add(matrix.get(i,j).get(0)));
            }
        }
        return new MatrixOnZ(body_new);
    }
    ...
    public Matrix substruct(Matrix matrix) {
        ...
        for(int i=0;i<getCountOfLine();i++){
            ArrayList<Fraction> part_new = new ArrayList<>();
            for(int j=0;j<getCountOfColumn();j++){
                part_new.add(this.get(i,j).get(0).substruct(matrix.get(i,j).get(0)));
            }
        }
        return new MatrixOnZ(body_new);
    }
    ...
    protected Matrix multyplyRecursive(Matrix matrix){
        ...
        Matrix R = MatrixOnZ.sizeOf(n,n);

        if (n == 1)
            R.set(0,0,get(0,0).multiply(matrix.get(0,0)));
        else {
    ...
    public Polynom det() {
        ...
        return new PolynomOnZ().addBack(det);
    }
    ...
    public static MatrixOnZ sizeOf(int countOfLine,int countOfColumn){
